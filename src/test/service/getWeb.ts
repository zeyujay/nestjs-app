 ...
 * @LastEditTime: 2023-03-25 00:23:27
...
const getWeb = async function (id, type) {
  ...
  if (type === 4) {
    ...
    const li: any = await page.waitForSelector('.game-list>ul>li:nth-child(1)');
  ...
  const detailData: any = await page.evaluate(async () => {
    ...
    const list = Array.prototype.filter.call(info.childNodes, function (item) {
      console.log(item);
        ...
        switch (item.textContent) {
          ...
          case '上映日期:':
            obj['上映日期'] = Array.prototype.map
              .call(
                info.querySelectorAll('span[property="v:initialReleaseDate"]'),
                function (item) {
                  return item.textContent;
                },
              )
